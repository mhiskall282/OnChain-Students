...
A connection cycle started for provider with name: ${this.name}`);
      ...
      refresh(interval = this.interval) {
        if (dev) console.log(`Reconnect queued for ${(interval / 1e3).toFixed(2)}s in the future`);
  ...
  "node_modules/eth-provider/provider/index.js"(exports, module) {
    ...
    var monitor = (provider) => {
      ...
      async function checkConnected() {
        if (provider.inSetup) return setTimeout(checkConnected, 1e3);
        try {
  ...
  "node_modules/eth-provider/connections/ws.js"(exports, module) {
    ...
    var WebSocketConnection = class extends EventEmitter {
      constructor(_WebSocket, url, options) {
        ...
        WebSocket2 = _WebSocket;
        setTimeout(() => this.create(url, options), 0);
      }
      create(url, options) {
        if (!WebSocket2) return this.onError(new Error("No WebSocket transport available"));
        try {
          this.socket = new WebSocket2(url, [], { origin: options.origin });
        } catch (e) {
      ...
      close() {
        if (this.socket && WebSocket2 && this.socket.readyState !== WebSocket2.CLOSED) {
          this.removeAllSocketListeners();
    ...
    module.exports = (WebSocket3) => (url, cb) => new WebSocketConnection(WebSocket3, url, cb);
  }
});
...
//# sourceMappingURL=browser-DE4F4R7W.js.map
