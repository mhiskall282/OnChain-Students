import {
  encode
} from "./chunk-VCMDR2GT.js";
import {
  getBytecode
} from "./chunk-HIIYDCM2.js";
import {
  formatBlock,
  hexToBytes,
  readContract
...
// node_modules/@thirdweb-dev/sdk/node_modules/thirdweb/dist/esm/extensions/modular/__generated__/ModularCore/read/getInstalledExtensions.js
...
async function download(options) {
  ...
  if (options.uri.startsWith("ar://")) {
    const { resolveArweaveScheme } = await import("./arweave-P3VUCEVJ.js");
  ...
  const res = await getClientFetch(options.client)(url, {
    ...
    requestTimeoutMs: (_i = (_h = (_g = options.client.config) == null ? void 0 : _g.storage) == null ? void 0 : _h.fetch) == null ? void 0 : _i.requestTimeoutMs
  });
  if (!res.ok) {
...
async function resolvePluginPatternAddresses(contract) {
  try {
    const { readContract: readContract2 } = await import("./read-contract-M6WFMMFB.js");
...
async function estimateGas(options) {
  ...
  const promise = (async () => {
    ...
    const { encode: encode2 } = await import("./encode-CUZ3V3DF.js");
    ...
    const [{ getRpcClient: getRpcClient2 }, { eth_estimateGas }] = await Promise.all([
      import("./rpc-3GYY5CKS.js"),
      import("./eth_estimateGas-VIIXEGNK.js")
    ]);
...
async function toSerializableTransaction(options) {
  ...
  let [data, nonce, gas, feeData, to, accessList, value] = await Promise.all([
    ...
    (async () => {
      ...
      return from ? await import("./eth_getTransactionCount-NIYCHKTZ.js").then(({ eth_getTransactionCount }) => eth_getTransactionCount(rpcRequest, {
        address: from,
        blockTag: "pending"
      })) : void 0;
    })(),
...
//# sourceMappingURL=chunk-CY2A75ND.js.map
